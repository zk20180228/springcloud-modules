...
public class Config02 implements EnvironmentAware {
    ...
    public void initUser() throws NacosException {
        ...
        String dataId = environment.getProperty("nacos.config.dataId02");
        String groupId = Optional.ofNullable(environment.getProperty("nacos.config.groupId02")).orElse(Constants.DEFAULT_GROUP);
        log.info("config02---user-->dataId="+dataId+",groupId="+groupId);
        //获取配置，并添加监听
        String config = configService.getConfigAndSignListener(dataId, groupId, 3000, new AbstractListener() {
            @Override
